...
def main():
    with st.sidebar:
        ...
        aai_api_key = st.text_input(
            "Enter AssemblyAI API Key", key="aai_api_key", type="password"
        )
        st.markdown(
            "[Get an OpenAI API Key](https://platform.openai.com/account/api-keys)"
        )
        st.markdown(
            "[Get an AssemblyAI API Key](https://www.assemblyai.com/app/account)"
        )
    ...
    if file is not None:
        ...
        if submit_btn:
            if not openai_api_key or not aai_api_key:
                st.warning(
                    "Please enter your OpenAI and AssemblyAI API keys to get started.",
                    icon="⚠",
                )
            else:
                aai_settings = {
                                ...
                                with st.expander("Cover Image Suggestion"):
                                    ...
                                    if not huggingface_api_token:
                                        ...
                                        music_theme_response = music_theme_generator(
                                            lyrics_content=transcription_results[
                                                "text"
                                            ],
                                            openai_api_key=openai_api_key,
                                        )
