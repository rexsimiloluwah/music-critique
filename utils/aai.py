...
def upload_to_assemblyai(file_path: str, api_key: str) -> (Optional[str], Optional[str]):
    ...
    try:
        response = requests.post(
            UPLOAD_ENDPOINT, headers=create_aai_headers(api_key=api_key), data=read_file(file_path)
        )
...
def submit_transcription_job(audio_url: str, api_key: str, language_code: Optional[str]="en_us", settings: Optional[Dict[str, bool]]=TRANSCRIPTION_SETTINGS) -> (Optional[str], Optional[str]):
